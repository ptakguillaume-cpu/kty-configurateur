...
/* ============================================================================
 * 0. VARIABLES GLOBALES (DÉCLARÉES EN PREMIER POUR ÉVITER LES BUGS)
 * ============================================================================ */
    ...
    chargerReferencesExternes: async function() {
        ...
        if (localData) {
            try {
                ...
                console.log("✅ Références locales chargées.");
                return;
        ...
        try {
                ...
                jsonBrut.produits.forEach(item => {
                    if (item.type === 'unique') {
                        ...
                        codesConvertis[item.nom] = {
                            "9016": item.ref_9016 || "-", "7016": item.ref_7016 || "-", "9005": item.ref_9005 || "-", "anodise": item.ref_anodise || "-", "autre": item.ref_autre || "-"
                        };
                    }
                });
            ...
            console.log("✅ Références serveur chargées.");
        } catch (erreur) {
...
window.adapterFormulaire = function() {
    ...
    if (forme === 'L' && configDepart === '1') {
        toggleDisplay('divPosDepartL', true);
    } else {
        toggleDisplay('divPosDepartL', false);
    }
    ...
    if (isDbleCheck) {
        ...
        if (valDouble.includes('_pleine')) {
            typePorteSelect.value = 'pleine';
        } else if (valDouble.includes('_alu')) {
            typePorteSelect.value = 'cadreAlu';
        }
    }
    ...
    if (!isDble) {
        if (typePorte === 'pleine') {
            const el = document.getElementById('selectSimplePleine');
            if(el) valSimple = el.value;
        } else {
            const el = document.getElementById('selectSimpleAlu');
            if(el) valSimple = el.value;
        }
    }
...
window.ajouterModuleMixte = function() {
    ...
    if(type === 'porte') {
        ...
        if(w < CONSTANTS.L_MIN_MODULE) { alert(`Min ${CONSTANTS.L_MIN_MODULE}mm`); return; }
    ...
    if (utilise + largeurAjout > L - deduction + 1) { alert("Dépassement longueur mur !"); return; }
...
window.remplirAutomatiquement = function() {
    ...
    if (type === 'pleine') {
        ...
        let ha = parseFloat(document.getElementById('mixteHauteurAllege').value)||0;
        for(let k=0; k<nb; k++) {
            configMurs[mur].push({type: type, largeur: wUnit, hAllege: ha});
        }
    }
...
window.calculerInventaire = async function() {
    ...
    // --- CALCUL PIECES & MONTANTS SPECIAUX ---
        ...
        keys.forEach(k => {
            if (inv[k] > 0) {
                ...
                if (typeof entry === 'string') {
                    ...
                    codeProduit = entry[ralValue] || "A DEFINIR";
                }
...
window.ajouterModuleMixte = function() {
    ...
    let largeurAjout = 0; let nouvModule = {};
    if(type === 'porte') {
        if(parseFloat(document.getElementById('qtePortes').value) < 1) { alert("Ajoutez une porte dans la config globale"); return; }
        largeurAjout = lp + CONSTANTS.EPAISSEUR_PROFIL; nouvModule = { type: 'porte' };
    } else {
        let w = parseFloat(document.getElementById('mixteLargeurModule').value)||0;
...
async function dessinerSceneGlobale(murs, forme, H, configs) {
    ...
    if(!b.isEmpty()) {
        const c = b.getCenter(new THREE.Vector3()); const sz = b.getSize(new THREE.Vector3());
        const maxDim = Math.max(sz.x, sz.y, sz.z); controls.target.copy(c); 
        const dist = maxDim * 1.2; const dir = new THREE.Vector3(0.8, 0.6, 1.0).normalize(); 
        currentScene.camera.position.copy(c).add(dir.multiplyScalar(dist));
        controls.update();
    }
}
